...
const compareSignal = (signalA: Signal, signalB: Signal): Intersection => {
  const intersection = signalA.relatives.reduce((r, relative) => {
    ...
    if (index > -1) {
      r.push([
        signalB.relatives[index],
        signalA.relatives.indexOf(relative),
        index,
      ])
    }
    return r
...
const initScanners = (input: Input[]): Scanner[] => {
  const scanners = input.map(
    ({ id, beacons }: Input): Scanner => ({
      id,
...
const alignScanner = (
  ...
  for (let line of data.intersection) {
    ...
    const d0 = data.here.pos.map(
      (_, i) => data.here.pos[i] - relativeHere.pos[i]
    )

    const relativeThere = scannerB.signals[line[1]]
    const d1 = data.there.pos.map(
      (_, i) => data.there.pos[i] - relativeThere.pos[i]
    )
    ...
    for (let signal of scannerB.signals) {
      ...
      signal.pos[0] =
        oldPos[0] * map[0] + oldPos[1] * map[3] + oldPos[2] * map[6]
      signal.pos[1] =
        oldPos[0] * map[1] + oldPos[1] * map[4] + oldPos[2] * map[7]
      signal.pos[2] =
        oldPos[0] * map[2] + oldPos[1] * map[5] + oldPos[2] * map[8]
    }
  ...
  for (let here of scanners) {
    for (let there of scanners) {
      max = Math.max(
        max,
        here.pos.reduce(
          (r, x, i) => r + Math.abs(here.pos[i] - there.pos[i]),
          0
        )
      )
    }
