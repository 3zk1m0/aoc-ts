...
const splitCubes = (ogCube:Cube, cutCube:Cube) => {
  ...
  if (cutCube.xMin > ogCube.xMin){
      cutCubes.push(createCube([ogCube.xMin, cutCube.xMin - 1], [ogCube.yMin, ogCube.yMax], [ogCube.zMin, ogCube.zMax]))
  }
  if (cutCube.xMax < ogCube.xMax){
      cutCubes.push(createCube([cutCube.xMax + 1, ogCube.xMax], [ogCube.yMin, ogCube.yMax], [ogCube.zMin, ogCube.zMax]))
  }

  let middleXRange:NPair = [Math.max(ogCube.xMin, cutCube.xMin), Math.min(ogCube.xMax, cutCube.xMax)];
  if (cutCube.yMin > ogCube.yMin){
      cutCubes.push(createCube(middleXRange, [ogCube.yMin, cutCube.yMin - 1], [ogCube.zMin, ogCube.zMax]));
  }
  if (cutCube.yMax < ogCube.yMax){
      cutCubes.push(createCube(middleXRange, [cutCube.yMax + 1, ogCube.yMax], [ogCube.zMin, ogCube.zMax]));
  }

  let middleYRange:NPair = [Math.max(ogCube.yMin, cutCube.yMin), Math.min(ogCube.yMax, cutCube.yMax)];
  if (cutCube.zMin > ogCube.zMin){
      cutCubes.push(createCube(middleXRange, middleYRange, [ogCube.zMin, cutCube.zMin - 1]));
  }
  if (cutCube.zMax < ogCube.zMax){
      cutCubes.push(createCube(middleXRange, middleYRange, [cutCube.zMax + 1, ogCube.zMax]));
  }
