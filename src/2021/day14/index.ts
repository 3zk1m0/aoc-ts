...
const run = (input, steps) => {
  ...
  counts[template[template.length-1]] = counts[template[template.length-1]] ? counts[template[template.length-1]]+1 : 1

  const max = Object.values<number>(counts).reduce((r,x) => Math.max(r,x), 0)
  const min = Object.values<number>(counts).reduce((r,x) => Math.min(r,x), Infinity)
