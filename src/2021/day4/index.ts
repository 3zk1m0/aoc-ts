...
const prepareInput = (rawInput: string) => {
  ...
  const tables = parts
    .slice(1)
    .map((x) =>
      x.split('\n').map((y) => y.match(/\d+/g).map((z) => parseInt(z, 10)))
    )
  return {
    numbers,
    tables,
...
const checkForWinner = (data) => {
  ...
  data.forEach((table, i) => {
    ...
    const rows = table.filter(
      (row) => row.filter((x) => x === 'X').length === 5
    )

    const columns = table.filter(
      (row, i, all) =>
        all.map((x) => x[i]).filter((x) => x === 'X').length === 5
    )
...
const part1 = (input: Input) => {
  ...
  while (true) {
    ...
    data = data.map((tables) =>
      tables.map((y) => y.map((x) => (x === number ? 'X' : x)))
    )
  ...
  const points = winner.reduce(
    (r, row) =>
      r + row.filter((num) => num !== 'X').reduce((rr, x) => rr + x, 0),
    0
  )
